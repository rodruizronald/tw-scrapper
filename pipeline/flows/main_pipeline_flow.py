...
async def main_pipeline_flow(
    ...
    stages_to_run: List[str] = ["stage_1", "stage_2", "stage_3", "stage_4"],
    ...
    try:
        ...
        if "stage_1" in stages_to_run:
            ...
            try:
                prompt_template_path = prompt_templates.get(
                    "stage_1", 
                    str(config.project_root / "input" / "prompts" / "job_title_url_parser.md")
                )
                ...
                # Update stage data for next stage
                ...
                if stage_1_results.get('total_jobs_saved', 0) == 0:
                    logger.warning("⚠️ No jobs were saved in Stage 1 - pipeline may not be effective")
        ...
        if "stage_2" in stages_to_run:
            logger.info("\n📄 STAGE 2: Job Description Extraction")
            logger.info("-" * 40)
            logger.warning("⚠️ Stage 2 not yet implemented - skipping")
            pipeline_results["stages_failed"].append("stage_2")
            pipeline_results["stage_results"]["stage_2"] = {
                "success": False,
                "error": "Stage 2 not yet implemented",
        ...
        if "stage_3" in stages_to_run:
            ...
            logger.info("-" * 40)
            logger.warning("⚠️ Stage 3 not yet implemented - skipping")
            pipeline_results["stages_failed"].append("stage_3")
            pipeline_results["stage_results"]["stage_3"] = {
                "success": False,
                "error": "Stage 3 not yet implemented",
                "error_type": "NotImplementedError",
            }
        
        # Stage 4: Technology Extraction (Future Implementation)
        if "stage_4" in stages_to_run:
            logger.info("\n🔧 STAGE 4: Technology Extraction")
            logger.info("-" * 40)
            logger.warning("⚠️ Stage 4 not yet implemented - skipping")
            pipeline_results["stages_failed"].append("stage_4")
            pipeline_results["stage_results"]["stage_4"] = {
                "success": False,
                "error": "Stage 4 not yet implemented",
                "error_type": "NotImplementedError",
            }
        ...
        pipeline_results.update({
            ...
            "pipeline_error_type": type(e).__name__,
        })
        
        logger.error(f"💥 Pipeline failed: {e}")
        _log_pipeline_summary(pipeline_results, logger)
        
        # Re-raise to let Prefect handle the failure
        raise
        ...
        for stage in stages_to_run:
            if stage in prompt_templates:
                ...
                if not template_path.exists():
                    raise ValueError(f"Prompt template not found for {stage}: {template_path}")


def _is_critical_stage_failure(stage: str, error: Exception) -> bool:
    ...
    # Other stages are less critical for now
    return False
...
def _log_pipeline_summary(results: Dict[str, Any], logger) -> None:
    ...
    logger.info(f"🎯 Stages Requested: {', '.join(results.get('stages_requested', []))}")
        ...
        for stage_name, stage_result in stage_results.items():
            if stage_result.get('success', False):
                ...
                if 'total_jobs_found' in stage_result:
                    logger.info(f"     📋 Jobs found: {stage_result['total_jobs_found']}")
                if 'total_jobs_saved' in stage_result:
                    logger.info(f"     💾 Jobs saved: {stage_result['total_jobs_saved']}")
            else:
