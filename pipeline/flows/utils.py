    ...
    try:
        ...
        for company_dict in companies_data:
            try:
                ...
                print(f"⚠️ Skipping invalid company data: {company_dict.get('name', 'unknown')} - {e}")
        ...
        raise ValueError(f"Error loading companies file: {e}")
...
def estimate_flow_duration(
    ...
    batches = (total_companies + max_concurrent - 1) // max_concurrent  # Ceiling division
