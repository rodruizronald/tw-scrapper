...
async def process_company_task(
    ...
    try:
        ...
        logger.error(
            f"❌ Validation error for {company_data.get('name', 'unknown')}: {e}"
        )
...
def validate_company_data_task(company_data: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        ...
        logger.error(
            f"❌ Validation failed for {company_data.get('name', 'unknown')}: {e}"
        )
        raise ValidationError(
            field="company_data", value=str(company_data), message=str(e)
        ) from e
...
def aggregate_results_task(company_results: list[dict[str, Any]]) -> dict[str, Any]:
    """
    ...
    non_retryable_failures = [
        r for r in failed_companies if not r.get("retryable", True)
    ]
    ...
    summary = {
        ...
        "average_processing_time": total_processing_time / len(company_results)
        if company_results
        else 0,
        "success_rate": len(successful_companies) / len(company_results)
        if company_results
        else 0,
    ...
    logger.info(
        f"❌ Failed: {len(failed_companies)} (retryable: {len(retryable_failures)}, non-retryable: {len(non_retryable_failures)})"
    )
