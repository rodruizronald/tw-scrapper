...
class Stage1Processor:
    ...
    async def process_company(self, company_data: CompanyData) -> ProcessingResult:
        ...
        try:
            ...
            logger.error(f"❌ {company_name}: Processing failed - {str(e)}")
            
            # Wrap in CompanyProcessingError if it's not already a pipeline error
            if not isinstance(e, (CompanyProcessingError, HTMLExtractionError, 
                                OpenAIProcessingError, FileOperationError, ValidationError)):
                raise CompanyProcessingError(company_name, e, "stage_1")
            
            return result
    
    async def process_companies(self, companies: List[CompanyData]) -> List[ProcessingResult]:
        """
        ...
        if self.config.stage_1.save_output:
            summary_path = self.file_service.create_processing_summary(processed_results)
    ...
    def _validate_company_data(self, company_data: CompanyData) -> None:
        ...
        if not company_data.job_board_selector:
            raise ValidationError("job_board_selector", "[]", "Job board selector cannot be empty")
        
        # Validate parser type
        try:
            ParserType[company_data.html_parser.upper()]
        except KeyError:
            raise ValidationError("html_parser", company_data.html_parser, "Invalid parser type")
    ...
    async def _extract_career_page_content(self, company_data: CompanyData) -> str:
        ...
        try:
            ...
            raise HTMLExtractionError(
                ...
                f"Unexpected error during HTML extraction: {str(e)}",
                company_data.name
            )
    
    async def _parse_job_listings(self, company_data: CompanyData, html_content: str) -> Dict[str, Any]:
        ...
        try:
            ...
            raise OpenAIProcessingError(
                f"Unexpected error during OpenAI processing: {str(e)}",
                company_data.name
            )
    
    def _process_job_listings(self, company_data: CompanyData, job_listings: Dict[str, Any]) -> List[JobData]:
        ...
        for job_data in job_listings.get("jobs", []):
            try:
                ...
                if not title or not url:
                    logger.warning(f"[{company_data.name}] Skipping job with missing title or URL: {job_data}")
                    continue
                ...
                logger.warning(f"[{company_data.name}] Error processing job data {job_data}: {e}")
                continue
    ...
    async def _filter_duplicate_jobs(self, company_data: CompanyData, jobs: List[JobData]) -> List[JobData]:
        """Filter out duplicate jobs based on historical signatures."""
        if not jobs:
            return jobs
        
        # Load historical signatures
        historical_signatures = self.file_service.load_historical_signatures(company_data.name)
