...
from ..utils.exceptions import OpenAIProcessingError, FileOperationError
...
class OpenAIService:
    ...
    async def parse_job_listings(
        ...
        for attempt in range(self.config.max_retries + 1):
            try:
                logger.info(f"{company_context}Sending content to OpenAI (attempt {attempt + 1})...")
                ...
                try:
                    ...
                    if attempt < self.config.max_retries:
                        ...
                        raise OpenAIProcessingError(error_msg, company_name, response_text)
    ...
    def _read_prompt_template(self, template_path: Path, company_name: Optional[str] = None) -> str:
        """Read prompt template from file."""
        try:
            with open(template_path, 'r', encoding='utf-8') as f:
                return f.read()
        except FileNotFoundError:
            raise FileOperationError("read", str(template_path), "File not found", company_name)
        except Exception as e:
