...
from parsers import ParserType, extract_by_selectors
from ..utils.exceptions import HTMLExtractionError
...
class HTMLExtractor:
    ...
    async def extract_content(
        ...
        for attempt in range(self.max_retries + 1):
            try:
                logger.info(f"{company_context}Extracting HTML content from {url} (attempt {attempt + 1})")
                ...
                for result in results:
                    if result.found and result.html_content:
                        ...
                        logger.debug(f"{company_context}Extracted content from selector: {result.selector}")
                    else:
                        logger.warning(f"{company_context}No content found for selector: {result.selector}")
                
                # Check if we got any content
                if not html_contents:
                    error_msg = f"No HTML content extracted from any selectors: {selectors}"
                    logger.warning(f"{company_context}{error_msg}")
                    
                    if attempt < self.max_retries:
                        logger.info(f"{company_context}Retrying in {self.retry_delay} seconds...")
    ...
    async def validate_url_accessibility(self, url: str, company_name: Optional[str] = None) -> bool:
        """
