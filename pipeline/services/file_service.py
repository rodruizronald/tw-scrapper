...
class FileService:
    ...
    async def save_jobs(
        ...
        try:
            ...
            error_msg = f"Failed to save jobs: {str(e)}"
    ...
    def load_historical_signatures(self, company_name: str) -> Set[str]:
        ...
        try:
            ...
            previous_day_base = self.base_output_dir.parent.parent / previous_timestamp / "pipeline_stage_4"
            sanitized_name = self.sanitize_company_name(company_name)
            previous_company_dir = previous_day_base / sanitized_name
            historical_file = previous_company_dir / "historical_jobs.json"
            
            if not historical_file.exists():
                logger.info(f"{company_context} No historical signatures found at {historical_file}")
                return set()
            ...
            logger.info(f"{company_context} Loaded {len(signatures)} historical signatures")
            return signatures
            
        except Exception as e:
            logger.warning(f"{company_context} Error loading historical signatures: {e}")
            return set()
    ...
    async def save_historical_signatures(
        ...
        try:
            ...
            logger.info(f"{company_context} Saved {len(signatures)} signatures to {output_path}")
            return output_path
            
        except Exception as e:
            error_msg = f"Failed to save historical signatures: {str(e)}"
            logger.error(f"{company_context} {error_msg}")
            raise FileOperationError("save", str(output_path), error_msg, company_name)
    
    def load_company_jobs(self, company_name: str, filename: str = "jobs_stage_1.json") -> List[Dict[str, Any]]:
        """
